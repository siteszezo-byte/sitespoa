// src/components/MateriaPrimaManager.jsx
...
function MateriaPrimaManager() {
  ...
  // --- NOVA FUNÇÃO PARA DELETAR UM PRODUTO ---
  const handleDeleteProduct = async (productId) => {
    // Adiciona uma confirmação para evitar exclusões acidentais
    if (
      window.confirm("Tem certeza de que deseja excluir esta matéria-prima?")
    ) {
      const { error } = await supabase
        .from("produtos")
        .delete()
        .eq("id", productId); // Deleta a linha onde o 'id' é igual ao productId

      if (error) {
        setMessage("Erro ao excluir: " + error.message);
      } else {
        // Remove o produto da lista na tela instantaneamente, sem precisar recarregar
    ...
    <div className="product-manager">
      ...
      <div className="form-section">
        ...
        <form onSubmit={handleAddProduct}>
          {/* ... (todos os inputs continuam aqui) ... */}
          <input
            type="text"
            name="nome_produto"
          ...
          <input
            ...
            value={newProduct.data_recebimento}
            onChange={handleInputChange}
          />
          <input
            type="text"
            name="temperatura_recebimento"
            placeholder="Temperatura de Recebimento"
            value={newProduct.temperatura_recebimento}
            onChange={handleInputChange}
          />
          <input
            type="text"
            ...
            value={newProduct.responsavel_recebimento}
            onChange={handleInputChange}
          />
      ...
      {/* Seção da Lista ATUALIZADA com o botão de excluir */}
              ...
              <li key={product.id} className="product-item">
                <div className="product-item-info">
                  ...
                  <div className="product-item-details">
                    <span>
                      Recebimento: {product.data_recebimento || "N/A"}
                    </span>
                    <span>
                      Temp.: {product.temperatura_recebimento || "N/A"}
                    </span>
                    <span>
                      Resp.: {product.responsavel_recebimento || "N/A"}
                    </span>
